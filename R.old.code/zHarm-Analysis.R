###########################################################
# load data and remove the quasi-periodic component of hs #
###########################################################
set.seed(1234)
...
thres.stat <- thres.orig - max(p.clim$lg.hs.p)  #candidates threshold

source("R.functions/ExtractPOTpp.R")
pots <- ExtractPOTPointProcess(p.clim$lg.hs.r, thres.stat)
...
# TODO: here I can add the same analysis done for the non statationary time series
# regarding the 
...
#####
#  TODO: I can try to use a geometric decay function 
#        instead of the empirical one
#####

####################################################
# Part 3: Generate a sample                        #
####################################################
#compute the interval width of lg.hs.p for later use
...
while(n.str < future){
  ...
  gap[i]  <- max(as.integer(r.Approx.Empirical(1, xfit,yfit)),1)  #time to next storm
  
# select a storm with similar value of  lg.hs.p for the corresponding date
